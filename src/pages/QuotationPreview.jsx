...
import { FileDown, Edit, Trash2, CheckCircle, Clock, Send, XCircle, ArrowLeft, FileText, ClipboardList, FileCheck, Zap, ChevronDown, Layers } from 'lucide-react'
...
function CommonInfoBlock({ common, setCommon, showSerial, showAppNo }) {
    ...
    <div style={{ background: '#f0f7ff', border: '1px solid #bfdbfe', borderRadius: 8, padding: '12px 14px', marginBottom: 12 }}>
      ...
      {showSerial && (
        <div style={{ marginTop: 10 }}>
          <label style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--gray-600)', display: 'block', marginBottom: 3 }}>Module Serial Numbers (one per line)</label>
          <textarea className="form-input" style={{ fontSize: 12.5, padding: '6px 10px', width: '100%', height: 80, resize: 'vertical' }}
...
export default function QuotationPreview({ navigate, quoteId }) {
  ...
  const [common, setCommon] = useState({
    ...
    serialNumbers: '', applicationNumber: ''
  })
  ...
  const [netMeterData, setNetMeterData] = useState({ agreementDate: '' })
  ...
  function openDocModal(type) {
    ...
    const cap = quote.systemCapacity?.toFixed(2) || ''
    if (type === 'wcr') {
      setWcrData(d => ({
        ...d,
    ...
    if (type === 'dcr') setDcrData(d => ({ ...d, panelMake: d.panelMake || pm }))
      ...
      <div className="preview-wrapper">
        ...
        <div style={{
          ...
          {[
            ...
            { label: 'Capacity', value: `${quote.systemCapacity?.toFixed(2)} kW` },
            { label: 'Phase', value: quote.phase },
        ...
        <div style={{ padding: '24px 32px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {[
                  ...
                  ['Mounting Structure', quote.mountingStructure, '1 Set'],
                  ['Wiring & LA Kit', quote.wiringKit, '1 Set'],
                  ['Installation Type', quote.installationType || 'Rooftop', 'Complete'],
                  ['System Capacity', `${quote.systemCapacity?.toFixed(3)} kW (${quote.panelCount} × ${quote.panelWattage} Wp)`, '—'],
                ].map(([comp, spec, qty], i) => (
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: 'var(--navy)', color: 'white' }}>
                  <th style={{ padding: '8px 10px', textAlign: 'left', fontSize: 11 }}>Description</th>
                  <th style={{ padding: '8px 10px', textAlign: 'center', fontSize: 11 }}>Qty</th>
                  <th style={{ padding: '8px 10px', textAlign: 'right', fontSize: 11 }}>Unit Price</th>
                  <th style={{ padding: '8px 10px', textAlign: 'right', fontSize: 11 }}>Amount</th>
                </tr>
              </thead>
              <tbody>
                {[
                  { desc: `Solar Panel — ${quote.panel}`, qty: quote.panelCount, unit: `${inr(quote.panelCostPerUnit)}/unit`, amount: (quote.panelCostPerUnit || 0) * (quote.panelCount || 0) },
                  { desc: `Inverter — ${quote.inverter}`, qty: 1, unit: inr(quote.inverterCost), amount: quote.inverterCost },
                  { desc: `Structure — ${quote.mountingStructure}`, qty: 1, unit: inr(quote.structureCost), amount: quote.structureCost },
                  { desc: `Wiring & LA — ${quote.wiringKit}`, qty: 1, unit: inr(quote.wiringCost), amount: quote.wiringCost },
                  { desc: 'Installation & Commissioning', qty: 1, unit: inr(quote.installationCharges), amount: quote.installationCharges },
                  ...
                  <tr key={i} style={{ background: i % 2 === 0 ? 'white' : 'var(--gray-50)' }}>
                    <td style={{ padding: '8px 10px', border: '1px solid var(--gray-200)' }}>{row.desc}</td>
                    <td style={{ padding: '8px 10px', border: '1px solid var(--gray-200)', textAlign: 'center' }}>{row.qty}</td>
                    <td style={{ padding: '8px 10px', border: '1px solid var(--gray-200)', textAlign: 'right' }}>{row.unit}</td>
                    <td style={{ padding: '8px 10px', border: '1px solid var(--gray-200)', textAlign: 'right', fontWeight: 600 }}>{inr(row.amount)}</td>
                  </tr>
                ))}
                {/* Subtotal */}
                <tr style={{ background: 'var(--gray-100)' }}>
                  <td colSpan={3} style={{ padding: '8px 10px', border: '1px solid var(--gray-200)', textAlign: 'right', fontWeight: 600 }}>Subtotal</td>
                  <td style={{ padding: '8px 10px', border: '1px solid var(--gray-200)', textAlign: 'right', fontWeight: 700 }}>{inr(quote.subtotal)}</td>
                </tr>
                {quote.gstAmount > 0 && (
                  <tr style={{ background: 'var(--gray-100)' }}>
                    <td colSpan={3} style={{ padding: '8px 10px', border: '1px solid var(--gray-200)', textAlign: 'right', fontWeight: 600 }}>GST ({quote.gstPercent}%)</td>
                    <td style={{ padding: '8px 10px', border: '1px solid var(--gray-200)', textAlign: 'right', fontWeight: 700 }}>{inr(quote.gstAmount)}</td>
                  </tr>
                )}
                <tr style={{ background: 'var(--gray-200)' }}>
                  <td colSpan={3} style={{ padding: '9px 10px', border: '1px solid var(--gray-300)', textAlign: 'right', fontWeight: 700, fontSize: 14 }}>Total System Cost</td>
                  <td style={{ padding: '9px 10px', border: '1px solid var(--gray-300)', textAlign: 'right', fontWeight: 800, fontSize: 14 }}>{inr(quote.totalCost)}</td>
                </tr>
                {/* Subsidy */}
                <tr style={{ background: '#D1FAE5' }}>
                  <td colSpan={3} style={{ padding: '8px 10px', border: '1px solid #6EE7B7', textAlign: 'right', fontWeight: 600, color: '#065F46' }}>
                    Less: PM Surya Ghar Muft Bijli Yojana (Govt. Subsidy)
                  </td>
                  <td style={{ padding: '8px 10px', border: '1px solid #6EE7B7', textAlign: 'right', fontWeight: 700, color: '#065F46' }}>
                    − {inr(quote.subsidy)}
                  </td>
                </tr>
                {/* Net Payable */}
                <tr style={{ background: '#FEF3C7' }}>
                  <td colSpan={3} style={{ padding: '11px 10px', border: '1px solid #FCD34D', textAlign: 'right', fontWeight: 800, color: '#92400E', fontSize: 15 }}>
                    NET PAYABLE AMOUNT
                  </td>
          ...
          <div style={{ textAlign: 'center', marginTop: 14, fontSize: 10.5, color: 'var(--gray-400)', borderTop: '1px solid var(--gray-100)', paddingTop: 10 }}>
      ...
      {docModal === 'wcr' && (
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 620, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <CommonInfoBlock common={common} setCommon={setCommon} showSerial showAppNo={false} />
      ...
      {docModal === 'dcr' && (
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 580, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <CommonInfoBlock common={common} setCommon={setCommon} showSerial showAppNo />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 }}>
              {[
                { label: 'Panel Make', key: 'panelMake', placeholder: 'e.g. Adani' },
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 500 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <FormField label="Agreement Date" type="date" value={cfaData.agreementDate} onChange={v => setCfaData(d => ({ ...d, agreementDate: v }))} />
              <div>
      ...
      {docModal === 'netmeter' && (
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 500 }}>
            ...
            <CommonInfoBlock common={common} setCommon={setCommon} showSerial={false} showAppNo={false} />
            <FormField label="Agreement Date" type="date" value={netMeterData.agreementDate}
              onChange={v => setNetMeterData(d => ({ ...d, agreementDate: v }))} />
            <div className="modal-actions" style={{ marginTop: 16 }}>
              <button className="btn btn-outline" onClick={() => setDocModal(null)}>Cancel</button>
      ...
      {docModal === 'all' && (
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 540 }}>
            ...
            <CommonInfoBlock common={common} setCommon={setCommon} showSerial={true} showAppNo={true} />
